...
1. **header**：包含簽章使用的演算法，一般常見最簡單的是 **HS256**，也就是只有一組金鑰做雜湊來產生簽章，要驗證簽章就一定要這組金鑰；另外一種常見的也是今天主題的重點：**RS256**，RS256 採用非對稱加密的方式，也就是分別會有功私鑰兩組金鑰，通常會在 server 端使用私鑰來簽章，而 client 端則使用公鑰來驗證簽章是否正確。
